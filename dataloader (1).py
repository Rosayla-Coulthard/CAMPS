...
def retrieve_catalog(paper_type:str, prompt:str, savepath = None, UI = 'Name', gal_title = 'Galaxy',
                     table_num = None, paper_name = None, single_galaxy = None, gal_name = None):
    """All-in-one function to retrieve data from Vizier or Simbad and saves it to a JSON file.
    
    Args:
        paper_type (str): The source of the data, either "Vizier" or "Simbad".
    ...
    elif paper_type == "Simbad":
        catalog = Simbad.query_bibobj(prompt)
        catalog_choice = 0
        table_name = prompt
    else:
    ...
    table_info = {"Data source": paper_type, "Paper tag": catalog_name, "Tables used": [table_name]}
    ...
    for star in catalog:
        ...
        # Converting units for Simbad papers
        if paper_type == 'Simbad':
            ra = output[star_gal_name][star_name][cat_title]['RAJ2000']
            dec = output[star_gal_name][star_name][cat_title]['DEJ2000']
            coord = SkyCoord(ra, dec, unit=(u.hourangle, u.deg))
            ra_temp = np.float64(coord.ra.deg)
            dec_temp = np.float64(coord.dec.deg)
            output[star_gal_name][star_name][cat_title]['RAJ2000'] = ra_temp 
            output[star_gal_name][star_name][cat_title]['DEJ2000'] = dec_temp 
        loops += 1
