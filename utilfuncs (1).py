...
def star_crossmatch(gal1:dict, gal1_catalog:str, gal2:dict, gal2_catalog:str,
    ...
    idx, d2d, _ = gal1_coords.match_to_catalog_sky(gal2_coords)
...
def make_dataframe(data, include_meta=True):
    ...
    if include_meta:
        ...
        for cat_name in list(metadata_dict['Included titles'].keys()):
            ...
            if cat["Table info"]['Data source'] == 'Vizier':
                columns = cat['Columns']
                source = cat["Table info"]['Data source']
                Paper_tag = cat["Table info"]['Paper tag']
                tables_used = cat["Table info"]['Tables used']
                members = cat['Members']
            else:
                columns = cat['Columns']
                source = cat["Table info"]['Data source']
                Paper_tag = cat["Table info"]['Tables used'][0]
                tables_used = cat["Table info"]['Paper tag']
                members = cat['Members']
